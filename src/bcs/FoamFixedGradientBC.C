...
FoamFixedGradientBC::validParams()
{
  ...
  params.addParam<std::string>("diffusivity",
...
FoamFixedGradientBC::FoamFixedGradientBC(const InputParameters & parameters)
  : FoamBCBase(parameters), _diffusivity(parameters.get<std::string>("diffusivity"))
{
  // check that the diffusivity coefficient is a OpenFOAM scalar field
  if (!_diffusivity.empty() && !_mesh->foamHasObject<Foam::volScalarField>(_diffusivity))
...
FoamFixedGradientBC::imposeBoundaryCondition()
{
  ...
  for (auto subdomain : subdomains)
  {
    ...
    // If diffusivity is specified grad array is a flux, so result
    // must be divided by it
    if (!_diffusivity.empty())
    {
