...
class TestFunctionSolver(TestCase):
    ...
    def test_times(self):
        ...
        for time in time_dirs:
            ...
            assert np.allclose(T, time, atol=0, rtol=1e-16), (
                f"Max diff ({time}): {abs(T - time).max()}"
            )

            ref = 1.0 if time > time_dirs[0] else 0.0
            assert np.allclose(dTdt, ref, atol=0, rtol=1e-14), (
                f"Max diff ({time}): {abs(dTdt - ref).max()}"
            )
    ...
    def test_compare_reference(self):
        ...
        for time in time_dirs:
            ...
            assert np.array_equal(T_ref, T), (
                f"Max diff ({time}): {abs(T - T_ref).max()}"
            )
            assert np.array_equal(dTdt, dTdt_ref), (
                f"Max diff ({time}): {abs(dTdt - dTdt_ref).max()}"
            )
