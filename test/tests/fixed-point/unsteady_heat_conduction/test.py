...
class TestUnsteadyHeatConductionInInfiniteSystem(unittest.TestCase):
    ...
    def test_solid_fixed_point(self):
        ...
        for time in times:
            ...
            assert np.array_equal(solid_temp, solid_temp_ref), f"Max diff ({time}): {abs(solid_temp-solid_temp_ref).max()}"
    ...
    def test_fluid_fixed_point(self):
        ...
        for time in times:
            ...
            assert np.array_equal(temp, temp_ref), f"Max diff ({time}): {abs(temp-temp_ref).max()}"
