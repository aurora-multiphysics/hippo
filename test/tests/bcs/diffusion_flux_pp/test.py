...
class TestFoamBCDiffusionFluxPP(unittest.TestCase):
    """Test class for imposing fixed gradient BCs in Hippo."""

    def test_fixed_gradient_x(self):
        """
        Test case for imposing diffusion flux BC with postprocessor.

        Solves laplace equation at each time step with right BC being k\partial_x T = 2t
        where k is 2. The analytical solution is t*x.
        """
        case_dir = "foam/"
